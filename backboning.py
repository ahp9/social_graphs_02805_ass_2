...
def thresholding(table, threshold):
   """Return only the edges surpassing a significance threshold."""
   table = table.copy()
...
def write(table, network, method, folder):
   if not table.empty and "src" in table:
      table.to_csv(f"{folder}/{network}_{method}.csv", sep="\t", index=False)
   else:
...
def stability_corr(table1, table2, method="spearman", log=False, what="nij"):
   ...
   corr_table = corr_table[[f"{what}_x", f"{what}_y"]]
   if log:
      corr_table[f"{what}_x"] = np.log(corr_table[f"{what}_x"])
      corr_table[f"{what}_y"] = np.log(corr_table[f"{what}_y"])
   return corr_table[f"{what}_x"].corr(corr_table[f"{what}_y"], method=method)
...
def doubly_stochastic(table, undirected=False, return_self_loops=False):
   ...
   table2 = table.copy()
   original_nodes = len(set(table["src"]) | set(table["trg"]))
   table = pd.pivot_table(table, values="nij", index="src", columns="trg", aggfunc="sum", fill_value=0) + 0.001
...
def disparity_filter(table, undirected=False, return_self_loops=False):
   ...
   if undirected:
      ...
      table = table.merge(table_minvar, on="edge", suffixes=("", "_varmin"))
      table = table.drop_duplicates(subset=["edge"])
      table = table.drop("edge", axis=1)
      table = table.drop("score_max", axis=1)
      table = table.drop("variance_varmin", axis=1)
...
def high_salience_skeleton(table, undirected=False, return_self_loops=False):
   ...
   if undirected:
      ...
      table = table.merge(table_maxscore, on="edge", suffixes=("", "_sum"))
      table = table.drop_duplicates(subset=["edge"])
      table = table.drop("edge", axis=1)
      table = table.drop("score_sum", axis=1)
